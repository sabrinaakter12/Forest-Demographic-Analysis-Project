...
> All species, plot IDs, and treatment labels used in the scripts are **anonymized placeholders** (for example, `Species_A`–`Species_G`, `Treatment_Wet`, `Treatment_Dry`, generic plot IDs). The raw forest inventory dataset is **confidential** and not included; the code reproduces the complete analysis workflow 
